...
Java不能向c++里面那样方便的修改char的指针，所以得自己定义一个char类型的缓冲区，然后将里面读取到的数据拷贝到buf数组即可，需要注意Java里面Sring、任何数组类型、基本类型的参数传的都是副本，不能通过直接修改指针指向来解决问题，因为实参并没有改变，但对于数组可以修改里面的元素。
...
作者：su-qi-er
链接：https://leetcode-cn.com/problems/read-n-characters-given-read4/solution/mei-ren-xie-javade-wo-lai-ba-by-su-qi-er/
来源：力扣（LeetCode）
